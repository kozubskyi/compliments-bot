...
messagesController.get('/:type/:for', async (req, res, next) => {
  try {
    ...
    if (!messages.length)
      throw new HttpErrors.NotFound(`Повідомлень з параметрами ${JSON.stringify(req.params)} не знайдено у базі даних`)
...
messagesController.post('/all', async (req, res, next) => {
  try {
    const newMessages = await MessageModel.create(req.body.map((msg) => ({ ...msg, created: createDate() })))
